...
func TestPenultimatePlayerFoldsFromBlind(t *testing.T) {
	...
	var winner *player
	winner = h.winner()
	if (winner != nil) { t.Error("Hand should not yet have been won")}
...
func TestOneFoldsAndOneChecksBlind(t *testing.T) {
	...
	err = h.check(p1)
	if (err != nil) { t.Error(err)}
	if (p1.chips != (initial - smallBlind)) { t.Errorf("Player 1 should have %d chips after checking but has %d", initial - smallBlind, p1.chips)}
}
...
func TestCallBlindThenChecksToTheRiver(t *testing.T) {
	...
	// river
	if (len(h.cards) != 5) { t.Errorf("unexpected number of cards, got %d	", len(h.cards)) }
	// err = h.check(p1)
}
